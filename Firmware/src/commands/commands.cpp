...
// Internal imports
#include "system.h"
#include "packets/blackboxtelemetry.h"
...
void Commands::BlackBoxTelemetryCommand(System &sm, const RnpPacketSerialized &packet)
{
    // Generate response packet
    BlackBoxTelemetryPacket responsePacket(
        sm.timeService.getMillis(),
        sm.timeService.getEpochMillis(),
        static_cast<uint8_t>(sm.primarysd.getState()),
        sm.primarysd.getTotalSpace(),
        sm.primarysd.getUsedSpace());

    // Set response packet header
    responsePacket.header.source_service = sm.commandhandler.getServiceID();
    responsePacket.header.destination_service = packet.header.source_service;
    responsePacket.header.source = packet.header.destination;
    responsePacket.header.destination = packet.header.source;
